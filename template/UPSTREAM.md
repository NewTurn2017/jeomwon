...
- Removed telemetry package dependencies from app manifests:
  - `apps/app/package.json`: removed `@sentry/nextjs` and `@v1/analytics`
  - `apps/web/package.json`: removed `@v1/analytics`
...
1. Added missing `@v1/backend` workspace dependency to `apps/web/package.json`.
...
- bun 1.3 isolated 링커 대응: `bunfig.toml` hoisted 지정, `@v1/typescript`·tailwind 계열 workspace 의존 명시 선언
