...
| Path | What it is |
|---|---|
| `template/` | The project source, fully rebranded to jeomwon (originally derived from get-convex/v1; pin recorded in `docs/upstream-report.md`): `domain.config.ts`-driven agents (triage + 4), KakaoTalk-style chat widget, operator dashboard (calendar / seat grid widgets), React Email × 4, `bun setup` wizard |
...
Roadmap phases 0–7 are complete, plus a full UI redesign (starter branding removed end to end; template QA 8/8 and browser walkthroughs of both apps verified after the redesign). Skill-only installs are supported via the scaffold's remote template fallback — the live GitHub download activates once this repo is public. Remaining backlog: publicizing the repo, a fresh end-to-end rehearsal, and regenerating `samples/pension-stay`. License: `template/LICENSE.md` carries the upstream license; a root license for the kit itself is not yet chosen.
