...
검증 기대: template 변경 시 bun run qa(8게이트) 또는 verify 게이트로 확인.
```
...
- 데모의 Convex dev 배포: `hardy-bulldog-667`(프로젝트 `ai-webinar`). RESEND 실 키 설정됨, AGENT_RUNTIME=openai(그래서 실 챗은 issue C로 죽음, QA는 mock으로 8/8).
