...
const preflight = {
	name: "preflight",
	script: join(SCRIPT_DIR, "preflight.mjs"),
	args: [targetDir, ...nameParts, packPath, "--lang", childLanguage],
	env: baseEnv,
};
const preflightResult = await runStage(preflight);
if (!preflightResult.ok) {
	if (preflightResult.signal)
		fail(
			"child_signal",
			`preflight: ${preflightResult.signal}`,
			signalExitCode(preflightResult.signal),
		);
	if (preflightResult.error)
		fail("child_spawn", `preflight: ${preflightResult.error}`);
	process.exit(preflightResult.code);
}

const stages = [
	{
		code: "stage_initialize",
		name: "initialize",
