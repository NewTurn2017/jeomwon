...
- `scaffold.mjs` copies the local template or verified immutable archive into a staged target, validates the template/pack/capability/setup/QA handshake before publication, excludes dependency/build/env artifacts, replaces `@jeomwon/` with the project npm scope, and prints next steps only when run standalone. The installed default is `assets/jeomwon-template-v0.1.0.tar.gz`. Every included `template/` change must rebuild that asset, update both manifest hashes, and pass `bun "${CLAUDE_SKILL_DIR}/scripts/build-template-archive.mjs" --check`; CI rejects stale bytes. The only excluded basenames are `.DS_Store`, `.env.local`, `.next`, `.react-email`, `.turbo`, `.vercel`, `node_modules`, `next-env.d.ts`, and `qa-artifacts`. For an explicit archive, recover from `archive_checksum_missing` by setting `JEOMWON_TEMPLATE_ARCHIVE_SHA256=$(shasum -a 256 <archive> | awk '{print $1}')`; checksum mismatch means the archive must not be used. Bun's native archive API rejects unsafe members.
