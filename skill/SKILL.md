...
- `preflight.mjs <target-dir> <project-name> <domain-pack.json>` resolves canonical, skills-CLI, manual-symlink, explicit `JEOMWON_SKILL_DIR`, and compatibility `CLAUDE_SKILL_DIR` layouts; verifies the exact `bun@1.3.14` template pin, both immutable archive hashes, v0/v1 pack schema, an absent/empty safe target, and a genuinely offline-ready Bun cache. It never writes target bytes. Every failure prints a stable error code and exactly one recovery argv; `cache_not_ready` points to the explicit network-allowed `warm-cache.mjs` command.
