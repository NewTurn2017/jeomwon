...
- `bootstrap.mjs <target-dir> <project-name> <domain-pack.json>` is the standard one-command deterministic path — a thin sequencer that runs `scaffold.mjs`, then `inject.mjs`, then `verify.mjs`, resolving them as its own siblings so it works from a repo clone or an installed skill. Every script accepts `--help` and `--lang ko|en|auto`; explicit language wins over `JEOMWON_CLI_LANG`, `LC_ALL`, `LC_MESSAGES`, and `LANG`. Bootstrap propagates the resolved language. It is offline-only: it deletes an ambient `JEOMWON_QA_BASE_URL` from the verify step and never runs live QA or `bun setup`. On the first stage failure it preserves the child exit/signal status and prints one bracketed recovery argv block beginning with the public `bun` token. On success it prints exactly one `[NEXT next_steps]` block.
