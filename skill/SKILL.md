...
8. **Kit feature flags — each is a boolean; ask all three (they are unreachable otherwise).**
   - waitlist: "자리가 차면 대기자를 받고 취소가 나면 알릴까요?" → `features.waitlist`.
   - customerAccounts: "고객이 로그인해 자기 예약을 직접 확인하는 구조입니까?" → `features.customerAccounts`.
   - operatorCalendarCrud: "사장님이 캘린더에서 예약을 직접 등록·수정·삭제해야 합니까?" → `features.operatorCalendarCrud`. **예 ⇒ `adminWidget`은 반드시 `calendar`여야 합니다** (인젝터가 `operatorCalendarCrud: true` + 다른 위젯을 하드 실패시킴). 물으면 기록하고, 안 물으면 셋 다 `false`로 굳습니다.
