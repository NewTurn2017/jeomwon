...
8. **Optional kit feature flags — ask all three.**
   - waitlist: "자리가 차면 대기자를 받고 취소가 나면 알릴까요?" → `features.waitlist`.
   - operatorCalendarCrud: "사장님이 캘린더에서 예약을 직접 등록·수정·삭제해야 합니까?" → `features.operatorCalendarCrud`. **예 ⇒ `adminWidget`은 반드시 `calendar`여야 합니다**.
   - noShow: "운영자가 시작이 지난 확정 예약을 노쇼로 표시해야 합니까?" → `features.noShow`. 켜면 고객에게 보일 `copy.noShow` 문구도 받습니다.
...
- `preflight.mjs <target-dir> <project-name> <domain-pack.json>` resolves canonical, skills-CLI, manual-symlink, and explicit `CLAUDE_SKILL_DIR` layouts; verifies the exact `bun@1.3.14` template pin, both immutable archive hashes, v0/v1 pack schema, an absent/empty safe target, and a genuinely offline-ready Bun cache. It never writes target bytes. Every failure prints a stable error code and exactly one recovery argv; `cache_not_ready` points to the explicit network-allowed `warm-cache.mjs` command.
- `bootstrap.mjs <target-dir> <project-name> <domain-pack.json>` is the standard one-command deterministic path — a thin sequencer that runs `preflight.mjs`, `scaffold.mjs`, then `inject.mjs`, then `verify.mjs`, resolving them as its own siblings so it works from a repo clone or an installed skill. Every script accepts `--help` and `--lang ko|en|auto`; explicit language wins over `JEOMWON_CLI_LANG`, `LC_ALL`, `LC_MESSAGES`, and `LANG`. Bootstrap propagates the resolved language. It is offline-only: it deletes an ambient `JEOMWON_QA_BASE_URL` from the verify step and never runs live QA or `bun setup`. On preflight failure it preserves preflight's single recovery argv unchanged (`cache_not_ready` begins with `env` and runs `warm-cache.mjs`); on a later stage failure it preserves the child exit/signal status and prints one bracketed recovery argv beginning with the public `bun` token. On success it prints exactly one `[NEXT next_steps]` block.
- `scaffold.mjs` copies the local template or verified immutable archive into a staged target, validates the template/pack/capability/setup/QA handshake before publication, excludes dependency/build/env artifacts, replaces `@jeomwon/` with the project npm scope, and prints next steps only when run standalone. The installed default is the versioned path in `jeomwon-skill.json`. Every included `template/` change must advance the patch version and archive filename, rebuild that asset, update both manifest hashes, and pass `bun "${CLAUDE_SKILL_DIR}/scripts/build-template-archive.mjs" --check`; CI rejects stale bytes, and published release archives are never overwritten. The only excluded basenames are `.DS_Store`, `.env.local`, `.next`, `.react-email`, `.turbo`, `.vercel`, `node_modules`, `next-env.d.ts`, and `qa-artifacts`. For an explicit archive, recover from `archive_checksum_missing` by setting `JEOMWON_TEMPLATE_ARCHIVE_SHA256=$(shasum -a 256 <archive> | awk '{print $1}')`; checksum mismatch means the archive must not be used. Bun's native archive API rejects unsafe members.
- `inject.mjs` validates canonical domain-pack schema v1 and writes generated domain config plus the optional email sample. A schemaVersion-less exact legacy v0 object is purely migrated by adding v1 defaults; explicit unsupported versions fail closed. The scaffolded `jeomwonSeed.ts` remains the single seed source and is never regenerated by injection.
- Successful injection publishes receipt schema v3 last. It binds project/template/release hashes, canonical pack, and managed output hashes for local consistency; it is not a signature against a coordinated hostile rewrite.
- `verify.mjs` runs offline install, typecheck, lint, tests, then builds, and optionally QA when `JEOMWON_QA_BASE_URL` points at the running generated authenticated app (`apps/app`). `VERIFY PASS` is printed only after every selected gate succeeds.
